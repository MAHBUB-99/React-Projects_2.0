...
export default function CardDetails({ onClose }) {
  ...
  const handleRemoveFromCart = (event, movie) => {
    event.preventDefault();
    const newCartData = cartData.filter((cartItem) => cartItem.id !== movie.id);
                ...
                <div key={movie.id} className="grid grid-cols-[1fr_auto] gap-4">
                  ...
                  <div className="flex justify-between gap-4 items-center">
                    <button
                      ...
                      onClick={(event) => handleRemoveFromCart(event, movie)}
                    >
                      <img className="w-5 h-5" src={deleteurl} alt="" />
